            ...
            <td class="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4 text-right">
              <div class="flex flex-row h-full w-full">
                <svg xmlns="http://www.w3.org/2000/svg" height="16" width="16" viewBox="0 0 512 512"
                  ...
                  @click="openUpdateAdminModal(data.id)">
                  <path
...
export default {
  setup() {
    ...
    function deleteAdmin(id) {
      if(confirm('Yakin untuk menghapus admin ini?')){
        adminStore.deleteAdminUser(id)
      }
    }

    function openUpdateAdminModal(id) {
      adminStore.openUpdateAdminModal(id)
    }
    
    return {adminStore, deleteAdmin, openUpdateAdminModal}
  },
