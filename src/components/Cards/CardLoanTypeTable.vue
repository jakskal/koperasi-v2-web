            ...
            <td class="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-4 text-right">
              <div class="flex flex-row h-full w-full">
                <svg xmlns="http://www.w3.org/2000/svg" height="16" width="16" viewBox="0 0 512 512"
                  ...
                  @click="openUpdateLoanTypeModal(data.id)">
                  <path
  ...
  setup() {
    ...
    function deleteLoanType(id) {
      if(confirm('Yakin untuk menghapus tipe simpanan ini?')){
        loanTypeStore.deleteLoanType(id)
      }
    }

    function openUpdateLoanTypeModal(id) {
      loanTypeStore.openUpdateLoanTypeModal(id)
    }
    
    return {loanTypeStore, deleteLoanType, openUpdateLoanTypeModal}
  },
